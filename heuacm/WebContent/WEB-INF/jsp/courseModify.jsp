...
<div class="container">
	<div class="row">
        ...
        <div class="col-xs-8">
        <h1>更新课程</h1>
        <form action="courseModifyAction?id=${course.id}" 
        onsubmit="return validate_form();" method="post">
			...
			name="location" value="${course.location}"><br>
			<span>教师:</span><input type="text" class="form-control" id="teacher" 
			name="teacher" value="${course.teacher}"><br>
			<span>时间:</span>
			<div class="input-append date form_datetime">
		    	<input type="text" placeholder="点击选择" class="form-control" 
		    	value="<fmt:formatDate value="${course.dt}" type="both" pattern="yyyy-MM-dd HH:mm "/>"
		    	 id="dt" name="dt" readonly>
			...
			<span>可见:</span><select class="form-control" id="visible" name="visible">
							<option value="1" <c:if test="${course.visible==true}">
							selected="selected"</c:if> >是</option>
							<option value="0" <c:if test="${course.visible==false}">
							selected="selected"</c:if> >否</option>
							</select><br>
			<span>作者:</span><input type="text" class="form-control" id="username" name="username" 
			value="${currentuser.name}" required="required" autocomplete="off" readonly="readonly"><br>
