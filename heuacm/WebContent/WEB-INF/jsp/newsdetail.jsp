...
<div class="container">
	...
	<c:if test="${news.top==true}"><h2><font color="red"> <span class="glyphicon glyphicon glyphicon-star"></span>[置顶] ${news.title}</font></h2></c:if>
	<c:if test="${news.top==false}"><h2>${news.title}</h2></c:if>
	</div>
	<div class="col-xs-2"><br>
	<c:if test="${modify==true}"><a href="newsModify?id=${news.id}"><button type="button" class="btn btn-primary">更新公告</button></a></c:if>
	</div>
	<br><br><br><br><br>
	<div align="center" class="col-xs-12">访问数：${news.visit}&nbsp;&nbsp;&nbsp;&nbsp;发布时间：<fmt:formatDate value="${news.dt}" type="both" pattern="yyyy-MM-dd(EEEE) HH:mm "/>&nbsp;&nbsp;&nbsp;&nbsp;发布者：${author}</div>
	<br><br><br>
	<div class="col-xs-2"></div>
	<div class="col-xs-8">${news.content}</div>
</div>
